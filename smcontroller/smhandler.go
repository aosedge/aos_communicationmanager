...
func (handler *smHandler) overrideEnvVars(envVars cloudprotocol.OverrideEnvVars) (err error) {
	log.WithFields(log.Fields{"id": handler.config.NodeID}).Debug("Override env vars SM ")
...
func (handler *smHandler) processSmMessages() {
	defer func() { handler.closeChannel <- struct{}{} }()
...
func (handler *smHandler) processUpdateInstancesStatus(data *pb.UpdateInstancesStatus) {
	log.WithFields(log.Fields{"id": handler.config.NodeID}).Debug("Receive SM update instances status")
...
func (handler *smHandler) processNewInstanceState(data *pb.NewInstanceState) {
	...
	if err := handler.messageSender.SendInstanceNewState(cloudprotocol.NewState{
		...
		log.Errorf("Can't send service new state: %v", err)
	}
}
...
func (handler *smHandler) processMonitoring(data *pb.NodeMonitoring) {
	log.WithFields(log.Fields{"id": handler.config.NodeID}).Debug("Receive SM monitoring")
...
func (handler *smHandler) processOverrideEnvVarsStatus(envVarStatus *pb.OverrideEnvVarStatus) {
	...
	if err := handler.messageSender.SendOverrideEnvVarsStatus(
		...
		log.Errorf("Can't sed override env ears status: %v", err.Error())
	}
}
