...
func New(
	...
	if !insecureConn {
		...
		controller.certChan = ch
		go controller.processCertChange()
	}
...
func (controller *Controller) stopGRPCServer() {
...
func (controller *Controller) processCertChange() {
	for {
		select {
		case <-controller.certChan:
			controller.restartGRPCServer()
